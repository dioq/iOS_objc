...
- (IBAction)convert_btn_act:(UIButton *)sender {
    NSString *date_str = @"2017-4-10 17:15:10";
    //字符串转时间戳
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];// 创建一个时间格式化对象
    [dateFormatter setDateFormat:@"YYYY-MM-dd HH:mm:ss"]; //设定时间的格式
    
    NSDate *date = [dateFormatter dateFromString:date_str];//将字符串转换为时间对象
    NSLog(@"%@",date);
    
    // 新格式
    [dateFormatter setDateFormat:@"yyyy-MM-dd-HH-mm"];
    NSString *new_date_str = [dateFormatter stringFromDate:date];
    NSLog(@"%@",new_date_str);
}
