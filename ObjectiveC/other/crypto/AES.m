...
{
    // 把待加密 UTF-8的字符串转化成 NSData
    NSData *data = [originalStr dataUsingEncoding:NSUTF8StringEncoding];
    // 进行AES 加密
    data = [self AESWithData:data operation:kCCEncrypt mode:mode key:key keySize:keySize iv:iv padding:padding];
    // 将加密后的数据 转化成 base64 字符串
...
+ (NSString *)AESDecrypt:(NSString *)originalStr
                 ...
                 padding:(AESPadding)padding
{
    // base64 字符串 解码成 NSData数据
    NSData *data = [[NSData alloc] initWithBase64EncodedString:originalStr options:0];
    // AES 解密
    data = [self AESWithData:data operation:kCCDecrypt mode:mode key:key keySize:keySize iv:iv padding:padding];
    // 将解密后的 NSData 数据转换成 UTF-8的字符串
                ...
                padding:(AESPadding)padding
{
    ...
    bytesTotal += bufused;
    if (padding == CryptorPKCS7Padding) {
...
+ (NSData *)removeBitPaddingWithData:(NSData *)sourceData mode:(CCMode)mode operation:(CCOperation)operation andPadding:(AESPadding)padding
{
    int correctLength = 0;
    ...
    }else if (padding == CryptorZeroPadding && end == 0) {
    ...
    }else if ((padding == CryptorANSIX923 || padding == CryptorISO10126) && (end > 0 && end < blockSize + 1)){
        correctLength = (short)sourceData.length - end;
        //        if (padding == CryptorISO10126 || ( testByte[sourceData.length - end] == 0 && testByte[sourceData.length - 2] == 0)) {
        //            correctLength = (short)sourceData.length - end;
        //        }
    }
